...
/**
 * Проверяет, является ли ПРИХОД займом (деньги, которые бизнес взял в долг),
 * а не выручкой. ВАЖНО (бухгалтерия, 13.07.2026): «Фреш Кредит» — это НЕ заём,
 * а выручка: клиент купил в рассрочку через Фреш Кредит, а они переводят нам
 * полную сумму покупки. Поэтому такие приходы = обычный доход (pnl_category=NULL),
 * а не 'loan'. Реальных займов-приходов сейчас нет — функция возвращает false;
 * если появятся, добавить сюда точный паттерн контрагента-кредитора.
 */
function isLoan(_name: string | null | undefined): boolean {
  return false;
}
